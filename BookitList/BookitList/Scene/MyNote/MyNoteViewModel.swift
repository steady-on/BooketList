...
final class MyNoteViewModel: Cautionable {
    ...
    func searchNotes(for keyword: String) -> [Note] {
        ...
        let result = notes.where {
            ...
            || $0.book.title.contains(keyword, options: .caseInsensitive)
            || $0.book.originalTitle.contains(keyword, options: .caseInsensitive)
            || $0.book.authors.name.contains(keyword, options: .caseInsensitive)
        }
        return Array(result)
    }
}
